...
- `.codex` active worktree가 큰 비중을 차지하면 `aibris clean --dry-run` 경로를 우선 제안한다. 필터가 없고 유용한 Codex worktree 추천이 있으면 기본 guided review가 열리며, 저위험 항목을 기본 선택하고 사용자가 번호로 빼거나 추가하게 한다.
...
- **worktree(active, codex)**: "{source}/{project} active Codex 워크트리가 {count}개 ({size}) 있습니다. 기본 clean에서는 보호되지만, `aibris clean --dry-run`이 유용한 경우 guided review를 열어 저위험 추천/보호 항목을 같이 보고 선택할 수 있습니다. preview를 열까요?"
...
Active Codex worktree bloat를 정리하는 경우에는 guided flow를 먼저 사용한다:

```bash
# 저위험 기본 선택 + 번호 토글 + 삭제 없는 미리보기
...
- 필터 없는 `aibris clean --dry-run`은 유용한 Codex worktree 추천이 있을 때 기본 guided review를 연다. `--no-guide`는 classic audit/executor 경로를 강제하고, `--guide`는 Codex worktree review를 명시적으로 강제할 때만 사용한다
