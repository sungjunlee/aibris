...
- active worktree가 큰 비중을 차지하면 `aibris clean --dry-run` 경로를 우선
  제안한다. 필터가 없고 도구와 무관하게 검증된 active worktree cleanup
  unit이 256 MB 이상이거나 3개 이상이면, 추천이 0개여도 기본 guided
  review가 열린다.
...
- 등록된 Codex activity source는 session metadata, cwd, timestamp만 사용한다.
  대화 본문은 읽거나 요약하지 않으며, 그 evidence를 다른 tool row에
  적용하지 않는다.
- activity source가 등록되지 않은 tool은 exact reason
  `activity source not registered for this tool`인 `reviewable` row로
  시작한다. 기본 선택·추천·projected freed에는 포함하지 않고, 사용자가
  row를 명시적으로 토글한 경우에만 preview한다
...
> active worktree는 classic clean에서 기본 제외된다. guided worktree review는
> 모든 tool의 active unit을 보여주되 별도 증거 정책을 통과한 unit만 추천한다.
> activity source가 없는 tool은 처음부터 선택하지 않는다. `--include-active-worktrees`는
...
- **worktree(active)**: "{source}/{project}에 연결된 {tool} 워크트리가
  {count}개 ({size}) 있습니다. `active`는 최근 사용 의미가 아니므로
  `aibris clean --dry-run`의 recommended/reviewable/locked 근거를 확인해야
  합니다. guided preview를 열까요?" 현재 tool은 activity source가 없으면
  `activity source not registered for this tool`로 reviewable·초기 미선택
  상태가 되며, 사용자가 직접 토글해야 한다.
...
worktree 정리를 승인한 경우에만 사용하는 별도 분기다:
...
1. 현재 디렉토리 포함, dirty/untracked, Git evidence unavailable,
   등록된 activity source unavailable, named ref에서 도달 불가능한 detached
   HEAD, healthy registered source의 activity가 6시간 이내인 경우는 `locked`.
2. activity source가 등록되지 않은 tool의 안전한 unit은 exact reason
   `activity source not registered for this tool`인 `reviewable`이며 초기
   미선택이다. age를 바꿔도 추천·초기 선택·projected freed로 승격하지 않는다.
3. healthy activity evidence가 있는 unit 중 canonical Git common-dir별 최신
   3개는 `reviewable`.
4. 3일보다 젊거나 256 MB 미만인 안전 unit은 `reviewable`.
5. 나머지는 `recommended`.
...
- 필터 없는 `aibris clean --dry-run`은 tool과 무관하게 검증된 active
  worktree pressure가 256 MB 이상이거나 3 unit 이상이면 추천 0개여도
  guided review를 연다. `--no-guide`와 명시적 classic selector는 classic
  경로를 강제한다. `--guide`는 guided worktree review를 명시적으로
  강제하며 category가 생략된 경우에만 `worktree`를 기본값으로 쓰고
  `--tool codex`를 만들지 않는다. 명시적 `--tool`은 guided inventory를
  그대로 좁힌다
