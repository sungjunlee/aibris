...
> ai-logs, cursor 카테고리는 기본적으로 clean 대상에서 제외된다.
...
Docker 정리는 `docker system prune`으로 직접 실행한다:
...
# risky 카테고리 포함 (ai-logs, cursor 등)
...
|----------|-----------|------|-----------|
| `worktree` | ✅ 안전 | AI coding tool worktrees | codex, claude |
| `node_modules` | ✅ 안전 | npm project dependencies | node_modules |
| `build-cache` | ✅ 안전 | Go/Xcode/Gradle/npm/Cargo caches | build-cache |
| `other-cache` | ✅ 안전 | Python/uv pip caches | pip-cache |
| `ai-logs` | 🚫 `--risky` 필요 | AI tool session logs, file history, archived sessions | ai-logs, cursor |
...
- **ai-logs/cursor 카테고리**는 기본 clean에서 제외. 삭제하려면 `--risky` 필요
