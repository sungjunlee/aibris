...
- `.codex` active worktree가 큰 비중을 차지하면 `aibris clean --guide --dry-run` 경로를 우선 제안한다. guide는 저위험 항목을 기본 선택하고 사용자가 번호로 빼거나 추가하게 한다.
...
- **worktree(active, codex)**: "{source}/{project} active Codex 워크트리가 {count}개 ({size}) 있습니다. 기본 clean에서는 보호되지만, `aibris clean --guide --dry-run`으로 저위험 추천/보호 항목을 같이 보고 선택할 수 있습니다. guided preview를 열까요?"
...
- `clean --guide`는 Codex worktree 전용 선택형 검토 모드다. 필터를 생략하면 `worktree/codex`로 좁히고, 저위험 active Codex worktree를 기본 선택하되 삭제 전 dry-run/confirm을 유지한다
