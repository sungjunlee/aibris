...
| build-cache | build-cache | ✅ | `$GOCACHE` or `UserCacheDir/go-build` (Linux `~/.cache/go-build`, Darwin `~/Library/Caches/go-build`, Windows `%LocalAppData%\go-build`), `~/.gradle/caches/`, `~/.npm/_cacache/`, `~/.cargo/registry/`, `~/Library/Caches/Xcode/`, `~/Library/Caches/Homebrew/` (`brew cleanup --prune=all`), `~/Library/Developer/Xcode/DerivedData/`, `~/Library/Caches/CocoaPods/`, `~/.dartServer/` (never `~/.pub-cache`) |
