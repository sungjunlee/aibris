...
- 이미 발견한 valid linked member가 있으면 그 repo의 `.git/worktrees/*/gitdir`으로 scan root 안 sibling checkout을 추가한다. primary checkout(`.git` 디렉터리), 없는/prunable 경로, root 밖, 이미 visit한 owner 소속은 건너뛴다. `$HOME` 전체 git 저장소 재귀는 하지 않는다
