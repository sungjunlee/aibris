...
const (
	lastScanCacheSchemaVersion = scancache.SchemaVersion
	lastScanCacheMaxAge        = scancache.MaxAge
)

type lastScanCache = scancache.Document

func writeLastScanCache(roots []string, identity string, result *types.ScanResult, explicit bool) {
	scancache.Write(roots, identity, result, explicit)
}

func writeLastScanCacheForSelector(roots []string, identity, selector string, explicit bool, result *types.ScanResult) {
	scancache.WriteForSelector(roots, identity, selector, explicit, result)
}
