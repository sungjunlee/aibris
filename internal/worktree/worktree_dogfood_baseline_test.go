...
func TestEvidenceBasedReclamationBaseline(t *testing.T) {
	...
	var units []worktree.WorktreeCleanupUnit
	...
	for i, size := range hardSizes {
		...
		case i < 6:
			...
			unit.Members[0].Reason = worktree.GitEvidenceReason{Code: worktree.GitReasonDirtyWorktree, Description: "dirty or untracked files", WorktreePath: unit.Members[0].WorktreePath}
			unit.HardLocked = true
			unit.HardLockReasons = []worktree.GitEvidenceReason{unit.Members[0].Reason}
		case i == 6:
			unit.Members[0].Recoverable = false
			unit.Members[0].HardLocked = true
			unit.Members[0].Reason = worktree.GitEvidenceReason{Code: worktree.GitReasonDetachedHeadUnreferenced, Description: "detached HEAD not reachable from named ref", WorktreePath: unit.Members[0].WorktreePath}
			unit.HardLocked = true
			unit.HardLockReasons = []worktree.GitEvidenceReason{unit.Members[0].Reason}
		default:
			...
			unit.Members[0].Recoverable = false
			unit.Members[0].HardLocked = true
			unit.Members[0].Reason = worktree.GitEvidenceReason{Code: worktree.GitReasonEvidenceUnavailable, Description: "Git evidence unavailable", WorktreePath: unit.Members[0].WorktreePath}
			unit.HardLocked = true
			unit.HardLockReasons = []worktree.GitEvidenceReason{unit.Members[0].Reason}
		}
	...
	for i, size := range recommendedSizes {
		...
		switch i {
		case 0:
			unit.Members[0].Upstream = worktree.GitUpstreamMetadata{State: worktree.GitUpstreamNone}
		case 1:
			unit.Members[0].BranchRef = ""
			unit.Members[0].ContainingRemoteRefs = []string{"refs/remotes/origin/preserved"}
			unit.Members[0].Reason = worktree.GitEvidenceReason{Code: worktree.GitReasonDetachedHeadReachable, Description: "detached HEAD reachable from named ref", WorktreePath: unit.Members[0].WorktreePath}
		case 2:
	...
	policy := worktree.DefaultCleanupPolicy(now)
	plan := worktree.PlanWorktreeCleanup(units, policy)
	assertBaselineDecisionShape(t, plan, 8, 13, 11, 7)
	assertBaselineDecision(t, plan, "locked-00", worktree.DecisionLocked, 1)
	assertBaselineDecision(t, plan, "locked-06", worktree.DecisionLocked, 1)
	assertBaselineDecision(t, plan, "locked-07", worktree.DecisionLocked, 1)
	assertBaselineDecision(t, plan, "recommended-00", worktree.DecisionRecommended, 1)
	assertBaselineDecision(t, plan, "recommended-01", worktree.DecisionRecommended, 1)
	assertBaselineDecision(t, plan, "recommended-02", worktree.DecisionRecommended, 2)
	...
	for _, decision := range plan.Decisions {
		...
		case worktree.DecisionReviewable:
			if decision.Reasons[0].Code == worktree.DecisionReasonRepositoryRetention {
	...
	strict := worktree.PlanWorktreeCleanup(units, policy)
	assertBaselineDecisionShape(t, strict, 8, 0, 11, 20)
}

func baselineCleanupUnit(name, repository string, size int64, activity time.Time) worktree.WorktreeCleanupUnit {
	target := "/fixture/.codex/worktrees/" + name
	return worktree.WorktreeCleanupUnit{
		TargetPath:                  target,
		Size:                        size,
		Source:                      ".codex",
		Members:                     []worktree.GitWorktreeMember{baselineCleanupMember(target, repository)},
...
func baselineCleanupMember(path, repository string) worktree.GitWorktreeMember {
	return worktree.GitWorktreeMember{
		WorktreePath:            path,
		...
		Upstream:                worktree.GitUpstreamMetadata{State: worktree.GitUpstreamPresent, Ref: "refs/remotes/origin/main"},
		Recoverable:             true,
...
func assertBaselineDecisionShape(t *testing.T, plan worktree.CleanupPlan, locked, recommended, retained, held int) {
	t.Helper()
...
func assertBaselineDecision(t *testing.T, plan worktree.CleanupPlan, name string, class worktree.DecisionClass, members int) {
	t.Helper()
