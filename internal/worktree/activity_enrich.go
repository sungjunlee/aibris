...
type ActivityOptions struct {
	Index        *codexactivity.Index
	IndexOptions codexactivity.IndexOptions
...
func BuildWorktreeCleanupUnitsWithActivity(ctx context.Context, items []types.DebrisInfo) ([]WorktreeCleanupUnit, error) {
	...
	if err := EnrichActivity(ctx, units, items, ActivityOptions{}); err != nil {
		return nil, err
	}
	return units, nil
}

func EnrichActivity(ctx context.Context, units []WorktreeCleanupUnit, items []types.DebrisInfo, opts ActivityOptions) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	...
	if opts.Index != nil {
		...
		activity = codexactivity.LoadWithOptions(ctx, opts.IndexOptions)
	}
	if err := ctx.Err(); err != nil {
		return err
	}
	if activity.Worktrees == nil {
		activity.Worktrees = make(map[string]codexactivity.Worktree)
	}
	if activity.Members == nil {
		activity.Members = make(map[string]codexactivity.Worktree)
	}
	if opts.Runner == nil {
		opts.Runner = RunGitCommand
	}
