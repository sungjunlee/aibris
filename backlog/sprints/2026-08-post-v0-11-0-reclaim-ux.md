...
- [ ] #253 Hint --apfs-snapshots after strip/clean when local snapshots still hold blocks (~1hr)
- [ ] #254 Point scan next at a reclaim ladder, not only clean --dry-run (~1hr)
- [~] #256 Rewrite --apfs-snapshots copy: local snapshots are not Time Machine backups (~15min)

### Batch 2 — summaries that share Batch 1 files

- [ ] #255 Summarize strip planned vs freed vs kept reasons (~1hr)
- [ ] #258 Say review-only worktrees are not a cleanup command (~1hr)
...
- Batch 2 waits because #255 edits the same strip closer as #253 and #258 edits the same scan `next`/summary as #254.
